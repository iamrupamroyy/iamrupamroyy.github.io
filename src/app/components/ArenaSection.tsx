...
import { Activity, ExternalLink, User, Target, CheckCircle2, Shield, Hash, BarChart } from 'lucide-react';
    ...
    >
      ...
      <div className="p-8 flex flex-col gap-8 flex-1">
         ...
         <div className="grid grid-cols-1 gap-4">
            {/* Detailed Rank/Rating Info */}
            <div className="p-4 border border-dashed flex items-center justify-between group-hover:border-[var(--cyan)] transition-colors" style={{ borderColor: 'var(--border-color)', backgroundColor: 'rgba(255,255,255,0.02)' }}>
               <div className="flex items-center gap-3">
                  <BarChart size={18} style={{ color: data.color }} />
                  <div className="flex flex-col">
                     <span className="text-[8px] font-mono opacity-40 uppercase tracking-tighter">Current_Status</span>
                     <span className="text-sm font-bold font-mono uppercase" style={{ color: 'var(--fg)' }}>{data.rank}</span>
                  </div>
               </div>
               {data.isLive && <div className="text-[8px] font-mono px-2 py-0.5 border border-[var(--green)] text-[var(--green)] uppercase animate-pulse">Live</div>}
            </div>

            {/* Performance Panel */}
            <div className="p-4 border border-dashed flex items-center gap-3 group-hover:border-[var(--cyan)] transition-colors" style={{ borderColor: 'var(--border-color)', backgroundColor: 'rgba(255,255,255,0.02)' }}>
               <Shield size={18} style={{ color: data.color }} />
               <div className="flex flex-col">
                  <span className="text-[8px] font-mono opacity-40 uppercase tracking-tighter">Performance_Tier</span>
                  <span className="text-sm font-bold font-mono uppercase" style={{ color: 'var(--fg)' }}>Top {data.percent}% Worldwide</span>
               </div>
            </div>
         </div>
